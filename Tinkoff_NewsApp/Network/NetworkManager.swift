...
class NetworkManager {
    ...
    private let headers = "top-headlines?country=ru&page="
    private let apiKey = "cc4981b5a8d143c4a13bf5dae5626be7"
   
    
    func getNews(pageNumber: Int, completion: @escaping (Result<[News]?,Error>)->Void) {
        let urlString = "\(baseUrl)\(headers)\(pageNumber)&apiKey=\(apiKey)"
